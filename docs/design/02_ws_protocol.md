...
- `ROOM_JOIN_REJECTED`
  ...
  - 備考: `client_version` が送信されていて最小対応版未満の場合、`reason` には `CLIENT_VERSION_UNSUPPORTED: ...` を返す
- `ROOM_UPDATED`
  - payload: `{ room_state_snapshot: RoomStateSnapshot }`
- `ROOM_CLOSED`
...
- ROOM_JOIN: `client_version` が送信されている場合は `client_version >= MIN_SUPPORTED_CLIENT_VERSION` を必須とし、未満なら `ROOM_JOIN_REJECTED` を返す
